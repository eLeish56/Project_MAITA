    ...
    $(document).ready(function() {
      ...
      $('#filter_by_date').on('click', function() {
        ...
        if (new Date(end_date) < new Date(start_date)) {
          return toastr.warning('Tanggal akhir tidak boleh lebih kecil dari tanggal awal');
        }

        // Show loading state
        $('#report').html('<div class="text-center p-5"><i class="fas fa-spinner fa-spin fa-2x"></i><br><p class="mt-2">Memuat data...</p></div>');
        
        // Disable filter button
        var $filterBtn = $('#filter_by_date');
        ...
        $.ajax({
          ...
          error: function(xhr, status, error) {
            ...
            $('#report').html('<div class="alert alert-danger">'+
              '<h5 class="alert-heading">Terjadi Kesalahan</h5>'+
              '<p>Gagal memuat data. Silakan coba lagi atau hubungi administrator.</p>'+
              '<hr>'+
